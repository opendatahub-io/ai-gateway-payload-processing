...
func TestE2E(t *testing.T) {
	...
	ginkgo.RunSpecs(t, "ExternalProvider/ExternalModel E2E Suite")
}
...
`, nsName))
	...
	ginkgo.By("Creating ExternalProvider and ExternalModel resources")
	for _, p := range providers {
		createProviderResources(p)
	}

	ginkgo.By("Creating multi-provider model for traffic splitting tests")
	createMultiProviderModel()

	ginkgo.By("Waiting for controller to create networking resources + plugin sync")
	waitForControllerResources()
	waitForMultiProviderRoute()
}
...
// waitForControllerResources waits for the controller to create HTTPRoutes
// for all providers (proves the reconciler is working).
func waitForControllerResources() {
	for _, p := range providers {
		gomega.Eventually(func() bool {
			cmd := exec.Command("kubectl", "get", "httproute", p.ModelName,
				"-n", nsName, "--no-headers", "--ignore-not-found")
			out, err := cmd.Output()
			return err == nil && len(strings.TrimSpace(string(out))) > 0
		}, 60*time.Second, 2*time.Second).Should(gomega.BeTrue(),
			fmt.Sprintf("HTTPRoute %s not created by controller within timeout", p.ModelName))
	}
}

// --- Kubernetes helpers (same patterns as existing e2e) ---
