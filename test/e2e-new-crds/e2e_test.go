...
// createMultiProviderModel creates an ExternalModel that splits traffic
// 80/20 between the OpenAI and Anthropic providers (already created by
// createProviderResources).
func createMultiProviderModel() {
...
spec:
  externalProviderRefs:
    - ref:
        name: e2e-prov-openai
      targetModel: e2e-openai
      apiFormat: openai
      weight: 80
    - ref:
        name: e2e-prov-anthropic
      targetModel: e2e-anthropic
      apiFormat: anthropic
      weight: 20
`, nsName))
}

func waitForMultiProviderRoute() {
...
spec:
  externalProviderRefs:
    - ref:
      ...
      targetModel: %s
      apiFormat: %s
`, p.ModelName, nsName, p.ProviderName, p.ModelName, p.ProviderType))
}
...
var _ = ginkgo.Describe("ExternalProvider/ExternalModel E2E", ginkgo.Label("e2e", "new-crds"), func() {
	...
	ginkgo.When("multi-provider traffic splitting", ginkgo.Label("tier2", "multi-provider"), func() {
		ginkgo.It("should return 200 for multi-provider model", func() {
		...
		ginkgo.It("should split traffic across both providers", func() {
			...
			gomega.Expect(openaiCount).To(gomega.BeNumerically(">", 0),
				"OpenAI should receive some traffic")
			gomega.Expect(anthropicCount).To(gomega.BeNumerically(">", 0),
				"Anthropic should receive some traffic")
		...
		ginkgo.It("should rewrite model field to targetModel", func() {
			...
			gomega.Expect(ok).To(gomega.BeTrue())
			gomega.Expect(model).NotTo(gomega.Equal("multi-split"),
				"Response model should be targetModel, not the client-facing name")
		})
	})
})
